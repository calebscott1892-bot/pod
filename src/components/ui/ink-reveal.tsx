...
/**
 * Ink-reveal (adapted from a 21st.dev block).
 *
 * A canvas mask sits over an image and is "wiped" away by the cursor, revealing
 * the image underneath through a wobbling ink brush.
 *
 * Changes from the original for this codebase:
 *  - The mask defaults to the brand cream (#f0e3db) so it blends with the page
 *    rather than flashing near-white.
 *  - A hover/pointer/reduced-motion gate: on touch devices, coarse pointers, or
 *    when reduced motion is requested, the mask is never painted, so the image
 *    is fully visible instead of hidden behind an un-wipeable overlay. The
 *    effect is purely decorative — the underlying image should stand on its own.
 */
interface InkRevealProps {
  /** RGB color of the mask overlay. Defaults to brand cream. */
...
export default function InkReveal({
  ...
  // Only wipe on hover-capable, fine-pointer devices without reduced-motion;
  // otherwise leave the canvas transparent so the image shows plainly.
  const enabledRef = useRef(true);
  // Holds the latest `loop` so the rAF schedules through a ref instead of
  // referencing `loop` before its own declaration (self-recursive callback).
  ...
  useEffect(() => {
    ...
    hoverMq.addEventListener("change", resize);
    motionMq.addEventListener("change", resize);
    return () => {
      window.removeEventListener("resize", resize);
      hoverMq.removeEventListener("change", resize);
